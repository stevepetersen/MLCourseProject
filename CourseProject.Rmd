...
Let's average the in-sample accuracy for all of the models.  

```{r}
sum <- 0
for (i in 1:k) {
  sum <- sum + (sum(models[[i]]$finalModel$confusion[,6])/5)
}
avg <- sum / k

```

We achieve `r 1-avg` in-sample accuracy with our cross-validated models.  We expect out-of-sample accuracy to be lower.
...
With our cross-validated models.
